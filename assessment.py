...
def print_indices(items):
    ...
    for num in range(item_list_length):
        print num, items[num]
    ...
    # item_list_length = len(items)
    # for num in range(item_list_length):
    #     print '%d %s' % (num, items[num])

    # Not sure why the below didn't work. The test "Expected" 
    # looked the same as "Got"
...
def foods_in_common(foods1, foods2):
    ...
    **NOTE**: for this problem, you're welcome to use any of the
    ...
    intersection = foods1_set & foods2_set # Return set of words in both sets
...
def every_other_item(items):
    ...
    # return [item for item in items if items.index(item) == 0 
    #         or items.index(item) % 2 == 0]
...
def largest_n_items(items, n):
    ...
    items.sort()
    reversed_items = items[::-1] # Reverse item list
    largest_items = reversed_items[:n] # Pick out n largest numbers
    largest_items.sort() # Resort list of largest numbers in ascending order
